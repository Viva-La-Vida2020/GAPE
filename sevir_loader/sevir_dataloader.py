...
r"""
There are several preprocess methods for sevir
e.g. PREPROCESS_01 was utilized in earthformer
Simply utilize NONE when you need the raw data without any preprocessing
"""
PREPROCESS_SCALE_NONE = {'vis': 1,  # No preprocess and get raw data
                          'ir069': 1,
                          'ir107': 1,
                          'vil': 1,
                          'lght': 1}
PREPROCESS_OFFSET_NONE = {'vis': 0,
                        'ir069': 0,
                        'ir107': 0,
                        'vil': 0,
                        'lght': 0}
...
# sevir
SEVIR_ROOT_DIR = os.path.join("/mnt/data/public_datasets/", "SEVIR")
SEVIR_CATALOG = os.path.join(SEVIR_ROOT_DIR, "CATALOG.csv")
SEVIR_DATA_DIR = os.path.join(SEVIR_ROOT_DIR, "data")
SEVIR_RAW_SEQ_LEN = 49
# sevir-lr
SEVIR_LR_ROOT_DIR = os.path.join("/mnt/data/public_datasets/", "sevir_lr")
...
class SEVIRDataLoader:
    ...
    def __init__(self,
        ...
        '根据start_date和end_date从catalog中截取数据'
        if self.start_date is not None:
            self.catalog = self.catalog[self.catalog.time_utc > self.start_date]
        if self.end_date is not None:
            self.catalog = self.catalog[self.catalog.time_utc <= self.end_date]
        if self.datetime_filter:
            self.catalog = self.catalog[self.datetime_filter(self.catalog.time_utc)]  # (39866,21)

        if self.catalog_filter is not None:
            if self.catalog_filter == 'default':
                self.catalog_filter = lambda c: c.pct_missing == 0  # 过滤保留pct_missing==0
            self.catalog = self.catalog[self.catalog_filter(self.catalog)]  # (39210, 61)
    ...
    def _compute_samples(self):
        ...
        imgts = set(imgt)
        filtcat = self.catalog[ np.logical_or.reduce([self.catalog.img_type==i for i in imgt]) ]  # (39210,21) filter vil
        # remove rows missing one or more requested img_types
        filtcat = filtcat.groupby('id').filter(lambda x: imgts.issubset(set(x['img_type'])))  # (12038,21)
        ...
        filtcat = filtcat.groupby('id').filter(lambda x: x.shape[0]==len(imgt))  # (11906,21)
        self._samples = filtcat.groupby('id').apply(lambda df: self._df_to_series(df,imgt) )  # df:(11906,2)
        if self.shuffle:
    ...
    def _read_data(self, row, data):
        ...
        return data  # [1,384,384,49] 基于hf5读取的raw data
    ...
    def __next__(self):
        ...
        if self.preprocess:
            ...
            ret_dict = self.preprocess_data_dict_vil(data_dict=ret_dict, layout=self.layout)
    ...
    @staticmethod
    def preprocess_data_dict_vil(data_dict, layout='NHWT'):
        data = data_dict['vil']
        # max_val = data.max()
        data = torch.where(
            data <= 5,
            ...
            torch.where(
                ...
                torch.where(
                    data <= 254,
                    torch.exp((data - 83.9) / 38.9),
                    data  # 如果 X > 254 的情况未指定，此处可以根据需要替换为其他操作
                )
            )
        )
    ...
    def preprocess_data_dict(data_dict, data_types=None, layout='NHWT', rescale='01'):
        ...
        data_dict:  Dict[str, Union[np.ndarray, torch.Tensor]]
            preprocessed data
        """
        raise RuntimeError("Check this preprocess data dict!")
        if rescale is None:
            scale_dict = PREPROCESS_SCALE_NONE
            offset_dict = PREPROCESS_OFFSET_NONE
        elif rescale == 'sevir':
