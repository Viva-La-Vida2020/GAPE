...
DATASETS_DIR = "/mnt/data/public_datasets/"
...
class SEVIRTorchDataset(TorchDataset):
    ...
    def __init__(self,
        ...
        self.sevir_dataloader = SEVIRDataLoader(
            # data_types=["vil", "ir069", "ir107", "lght", "vis"],
            data_types=["vil"],
            seq_len=seq_len,
...
def get_sevir_datamodule(dataset_oc,
    ...
    dm = SEVIRLightningDataModule(
        ...
        batch_size=dataset_oc["batch_size"],
        layout=dataset_oc["layout"],
        output_type=np.float32,
        preprocess=True,
        rescale_method=dataset_oc["rescale_method"],
        verbose=False,
        # datamodule_only
...
class SEVIRLightningDataModule(LightningDataModule):
    ...
    def __init__(self,
        ...
        if dataset_name == "sevir":
            ...
            sevir_root_dir = os.path.join(DATASETS_DIR, "SEVIR")
            ...
            sevir_root_dir = os.path.join(DATASETS_DIR, "sevir_lr")
            catalog_path = os.path.join(sevir_root_dir, "CATALOG.csv")
            raw_data_dir = os.path.join(sevir_root_dir, "data")
            raw_seq_len = 25
    ...
    def prepare_data(self) -> None:
        if os.path.exists(self.sevir_root_dir):
            ...
            if self.dataset_name == "sevir":
                # download_SEVIR()
                raise RuntimeError("SEVIR data not found! You could use download_SEVIR() to download it")
            else:  # "sevir_lr"
                raise NotImplementedError

    def setup(self, stage=None) -> None:
        self.sevir_train = SEVIRTorchDataset(  # 35718
        ...
        self.sevir_val = SEVIRTorchDataset(  # 9060
            sevir_catalog=self.catalog_path,
            sevir_data_dir=self.raw_data_dir,
            raw_seq_len=self.raw_seq_len,
            split_mode="uneven",
            ...
            rescale_method=self.rescale_method,
            verbose=self.verbose, )
        self.sevir_test = SEVIRTorchDataset(  # 12159
            sevir_catalog=self.catalog_path,
            sevir_data_dir=self.raw_data_dir,
            raw_seq_len=self.raw_seq_len,
            split_mode="uneven",
