...
class Noise_Projector(nn.Module):
    ...
    def forward(self, x):
        x = self.conv_first(x)  # [1,64,16,16]
        x = self.L1(x)  # [1,128,16,16]
        x = self.L2(x)  # [1,256,16,16]
        x = self.L3(x)  # [1,512,16,16]
        x = self.L4(x)  # [1,1024,16,16]

        return x


class ProjBlock(nn.Module):  # L Block
